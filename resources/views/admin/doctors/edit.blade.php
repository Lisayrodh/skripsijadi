...
<div class="container" style="margin-top: 20px;">
    <h3>Edit Profile Dokter dan Perawat</h3>
    <div class="row justify-content-center">
        <div class="col-md-6">
            <form action="{{ route('doctors.update', $doctor->id_doctor) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="position" name="position" value="{{ $doctor->position }}" required>
                </div>
                <div class="form-group">
                    <label for="description">Deskripsi:</label>
                    <textarea class="form-control" id="description" name="description" required>{{ $doctor->description }}</textarea>
                </div>
                <div class="form-group">
                    <label for="photo">Foto:</label>
                    <img src="{{ asset('storage/'. $doctor->photo) }}" alt="Current Photo" width="100" height="100">
                    <input type="file" class="form-control" id="photo" name="photo">
                    <small>Upload a new photo (optional)</small>
                </div>
                <div class="form-group">
                    <label for="twitter">Twitter (URL):</label>
                    <input type="url" class="form-control" id="twitter" name="twitter" value="{{ $doctor->twitter }}">
                </div>
                <div class="form-group">
                    <label for="facebook">Facebook (URL):</label>
                    <input type="url" class="form-control" id="facebook" name="facebook" value="{{ $doctor->facebook }}">
                </div>
                <div class="form-group">
                    <label for="instagram">Instagram (URL):</label>
                    <input type="url" class="form-control" id="instagram" name="instagram" value="{{ $doctor->instagram }}">
                </div>
                <div class="form-group">
                    <label for="linkedin">LinkedIn (URL):</label>
                    <input type="url" class="form-control" id="linkedin" name="linkedin" value="{{ $doctor->linkedin }}">
                </div>
